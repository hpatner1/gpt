        ...
        if ($coin === '' || strlen($coin) > 25) {
            ...
            $stmt = $pdo->prepare(
                'INSERT INTO trades (
                    ...
                    risk_amount, position_size, rr_ratio, potential_profit, potential_loss, status, trade_date, created_at, updated_at
                ) VALUES (
                    :user_id, :coin_name, :account_balance, :risk_percent, :entry_price, :stop_loss_price, :take_profit_price,
                    :risk_amount, :position_size, :rr_ratio, :potential_profit, :potential_loss, :status, :trade_date, NOW(), NOW()
                )'
            );
            $stmt->execute([
